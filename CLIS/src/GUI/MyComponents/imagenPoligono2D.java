...
{	
	...
	{				
		...
		else
		{
			...
			imagen = new BufferedImage( radio + 1, radio + 1, BufferedImage.TYPE_INT_ARGB );			
			g = imagen.getGraphics();
		}
	...
	public static Image crearImagenOvalo( int x, int y, int width, int height, float thicknessBorder, Color colorBorder, Color colorRelleno, Image img )
	{
		...
		else
		{
			...
			imagen = new BufferedImage( width + 1, height + 1, BufferedImage.TYPE_INT_ARGB );			
			g = imagen.getGraphics();
		}
	...
	public static Image crearImagenArco( int x, int y, int width, int height, int startAngle, int arcAngle, float thicknessBorder, Color colorBorde, Color ColorRelleno, Image img )
	{
		...
		{	
			int w = width + 2*thick + 1;
			int h = height + 2*thick + 1;
		...
		{	
			g.setColor( ColorRelleno );
			g.fillArc( x + thick / 2 + 1, y + thick / 2 + 1, width, height, startAngle, arcAngle );
		}
		...
		g.setColor( cborde );
		((Graphics2D)g).setStroke( new BasicStroke( thicknessBorder ) );
		g.drawArc( x + thick / 2 + 1, y + thick / 2 + 1, width, height, startAngle, arcAngle );
	...
	{		
		...
		int[] xs = { t, ancho, ancho, t };
		int[] ys = { 0,  0, alto - t,  alto -t };
	...
	public static Image crearImagenTexto( int x, int y, String texto, FontMetrics fm, Color colorBorder, Color colorRelleno, Image img )
	{
		...
		if( colorBorder != null )
		{
			...
			g.drawString( texto, x-1, y + fm.getAscent());
			g.drawString( texto, x+1, y + fm.getAscent());
			g.drawString( texto, x, y + fm.getAscent()-1);
			g.drawString( texto, x, y + fm.getAscent()+1);
		}
		
		g.setColor( colorRelleno );
		g.drawString( texto, x, y + fm.getAscent());
