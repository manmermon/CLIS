...
public class coreControl extends Thread implements IControlLevel1
{
	...
	private outputDataFileControl ctrlOutputFile = null;
	...
	private void createControlUnits() throws Exception
	{
		...
		try
		{
			this.ctrlOutputFile = outputDataFileControl.getInstance();
			this.ctrlOutputFile.setControlSupervisor(this);
			this.ctrlOutputFile.start();
		}
	...
	public void disposeLSLDataPlot()
	{
		...
		{
			this.ctrLSLDataPlot.stopThread( IStoppableThread.ForcedStop );
			this.ctrLSLDataPlot = null;
		}
	}

	public void createLSLDataPlot( guiLSLDataPlot LSLDataPlot, LSLConfigParameters t )
	{
		try
		{
			if (this.ctrLSLDataPlot != null)
			...
			/*
			LSLDataPlot.setVisible( false );
			LSLDataPlot.removeAll();
			LSLDataPlot.setVisible( true );
			 */
			...
			for (int i = 0; i < results.length; i++)
			{
				...
				if ( ( info.type().equals( t.getDeviceType() ) ) 
						&& ( info.name().equals( t.getDeviceName() ) ) )
				{
					inletInfo = results[i];
				}
			}
			...
			if ( inletInfo != null)
			{
				...
				this.ctrLSLDataPlot = new outputDataPlot( LSLDataPlot, inletInfo, t );
		...
		{	
			...
			try
			{
				...
				{						
					...
					file = file + ConfigApp.getProperty( ConfigApp.LSL_OUTPUT_FILE_NAME ).toString();
					...
					if ( !DEV_ID.isEmpty() )
					{
						File folder = new File( ConfigApp.getProperty( ConfigApp.OUTPUT_FOLDER ).toString() );
						if( !folder.exists() )
						{
							if( !folder.mkdir() )
							{
						...
						pars.put( file, DEV_ID );
							
						this.ctrlOutputFile.addSubordinates( pars );
			...
			{
				ErrorException.showErrorException(guiManager.getInstance().getAppUI(), e.getMessage(), "Output File Error", ErrorException.ERROR_MESSAGE);
				e.printStackTrace();
			}
			catch (Error e)
			{
	...
	private void checkCorrectConfiguration() throws ConfigurationException
	{
		...
		this.warnMsg.setMessage( this.warnMsg.getMessage() + outFileMsg.getMessage(), outFileMsg.getWarningType() );
		...
		if( results.length > 0 )
		{
			...
			if( !selected )
			{
				this.warnMsg.setMessage(this.warnMsg.getMessage() + "Non one LSL devices selected.", WarningMessage.WARNING_MESSAGE );
			}
		}
	...
	private synchronized void preStartPhase() throws Exception
	{
		...
		this.ctrlOutputFile.toWorkSubordinates( new Tuple< String, String >( outputDataFileControl.ID_PARAMETER_FILE_FORMAT 
		...
		if (this.timersParameters.containsKey( this.PRE_TIMER ))
		{
			...
			this.ctrlOutputFile.toWorkSubordinates( new Tuple< String, Integer >( ctrlOutputFile.ID_PARMETER_SET_MARK,  
													(Integer)Commands.getOutputDataFileMark().get( Commands.TRIGGERED_EVENT_PRERUN ) ) );
	...
	private synchronized void showActivity(JPanel activityPanel)
	{
		try {
			if (this.ctrlOutputFile != null)
			{
				try
				{
					String key = Commands.TRIGGERED_EVENT_NEW_OPERATION;
					this.ctrlOutputFile.toWorkSubordinates(new Tuple< String, Integer>( ctrlOutputFile.ID_PARMETER_SET_MARK,  
							Commands.getOutputDataFileMark().get( key ) ) );
				}
	...
	private synchronized void startOrContinueActivity()
	{
		...
		try
		{
			...
			if ( (!this.sentFirstNewOperationEvent ) 
					...
					&& ( isShowNextActPhase ) )
			{
				Tuple<String, Integer> cmd = new Tuple<String, Integer>(ctrlOutputFile.ID_PARMETER_SET_MARK, 
						Commands.getOutputDataFileMark().get( Commands.TRIGGERED_EVENT_NEW_PHASE ) );

				this.ctrlOutputFile.toWorkSubordinates(cmd);
	...
	private synchronized void finishPhase()
	{
		if (this.timersParameters.containsKey( this.POST_TIMER ) )
		{
			try
			{
				...
				this.ctrlOutputFile.toWorkSubordinates(new Tuple< String, Integer>( ctrlOutputFile.ID_PARMETER_SET_MARK, 
						Commands.getOutputDataFileMark().get( Commands.TRIGGERED_EVENT_POSRUN ) ));
	...
	public synchronized void stopWorking( boolean nextTestBenchFile )
	{
				...
				|| this.isWaitingForStartCommand )
		{
			...
			if (this.ctrlOutputFile != null)
			{
				try
				{	
					String key = Commands.TRIGGERED_EVENT_TEST_ENDED;
					...
					this.ctrlOutputFile.deleteSubordinates( IStoppableThread.StopInNextLoopInteraction );
					
					if( this.ctrlOutputFile.isSavingData() )
					{
						guiManager.getInstance().setAppState( "Saving" );
					}
				}
				catch (Exception localException) 
				{
					guiManager.getInstance().setAppState( "" );
				}
				catch (Error localError) 
				{}
			}
				...
				if (timerEvent)
				{
					if (this.ctrlActivity.isAnswerPhase())
					{
						if (this.ctrlOutputFile != null)
						{
							try
							{
								String key = Commands.TRIGGERED_EVENT_TIMEOUT;
								this.ctrlOutputFile.toWorkSubordinates(new Tuple< String, Integer>( ctrlOutputFile.ID_PARMETER_SET_MARK, 
										Commands.getOutputDataFileMark().get( key ) ));
							}
							catch (Exception localException1) 
				...
				else if (this.ctrlActivity.isAnswerPhase())
				{
					if ( (Boolean)info )
					{
						if (this.ctrlOutputFile != null)
						{
							try
							{
								String key = Commands.TRIGGERED_EVENT_CORRECT_ANSWER;
								this.ctrlOutputFile.toWorkSubordinates(new Tuple< String, Integer>( ctrlOutputFile.ID_PARMETER_SET_MARK, 
										Commands.getOutputDataFileMark().get( key ) ));
							}
					...
					else
					{
						if (this.ctrlOutputFile != null)
						{
							try
							{
								String key = Commands.TRIGGERED_EVENT_INCORRECT_ANSWER;
								this.ctrlOutputFile.toWorkSubordinates(new Tuple< String, Integer>( ctrlOutputFile.ID_PARMETER_SET_MARK, 
										Commands.getOutputDataFileMark().get( key ) ));
							}
	...
	private class controlNotifiedManager extends AbstractStoppableThread implements INotificationTask
	{
		...
		protected void runInLoop() throws Exception
		{
			if (this.eventRegister.size() > 0)
			{
				...
				else if( event_type.equals( eventType.ALL_OUTPUT_DATA_FILES_SAVED ) )
				{
					guiManager.getInstance().setAppState( "Saved" );
